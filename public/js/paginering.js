const PAGINERING = {
    pagineer: (pagina, knoppen, aantalPaginas) => {
        ...
        // 2 of meer pagina's
        let ul = $('<ul>').addClass('pagination mt-2');
        ...
        if (pagina > 0)
            $(ul).append(
                $('<li>').addClass('page-item').append(
                    $('<a>').addClass('page-link').prop('href', '#').data('pagina', pagina - 1)
                        .append($('<i>').addClass('bi bi-chevron-double-left'))
                )
            );
        ...
        if (pagina > midden)
            $(ul).append(
                $('<li>').addClass('page-item').append(
                    $('<a>').addClass('page-link').prop('href', '#').text('...')
                    .prop('disabled', true)
                )
            );
        ...
        for (let paginaNo = start; paginaNo <= stop; paginaNo++) {
            ...
            $(ul).append(
                $('<li>').addClass('page-item').append(
                    $('<a>').addClass('page-link').prop('href', '#')
                        .data('pagina', paginaNo).text(paginaNo + 1).addClass(actief)
                )
            );
        }
        ...
        if (pagina < aantalPaginas - 1)
            $(ul).append(
                $('<li>').addClass('page-item').append(
                    $('<a>').addClass('page-link').prop('href', '#').data('pagina', pagina + 1)
                        .append($('<i>').addClass('bi bi-chevron-double-right'))
                )
            );
